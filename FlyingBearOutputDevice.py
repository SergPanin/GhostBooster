...
class FlyingBearOutputDevicePlugin(OutputDevicePlugin): #We need to be an OutputDevicePlugin for the plug-in system.
    ...
    def start(self):
        self.getOutputDeviceManager().addOutputDevice(FlyingBearOutputDeviceSD()) #Since this class is also an output device, we can just register ourselves.
        self.getOutputDeviceManager().addOutputDevice(FlyingBearOutputDeviceUSB()) #Since this class is also an output device, we can just register ourselves.
    ...
    def stop(self):
        self.getOutputDeviceManager().removeOutputDevice("flyingbear_output_device_sd") #Remove all devices that were added. In this case it's only one.
        self.getOutputDeviceManager().removeOutputDevice("flyingbear_output_device_usb") #Remove all devices that were added. In this case it's only one.

class FlyingBearOutputDeviceSD(OutputDevice): #We need an actual device to do the writing.
    def __init__(self):
        super().__init__("flyingbear_output_device_sd") #Give an ID which is used to refer to the output device.
        ...
        self.setShortDescription("Send To Printer SD") #This is put on the save button.
        self.setDescription("Send gcode to Ghost SD Drive via WIFI")
    ...
    def requestWrite(self, nodes, file_name = None, limit_mimetypes = None, file_handler = None, **kwargs):
        ...
        Logger.log("d", "preferred_mimetypes: %s", preferred_mimetypes)
    ...
    def do_snap(self,gfile):
        ...
        if m0 is not None:  #if not isinstance(m0,NoneType): #NoneType
            ...
            if m1:
                ...
                f.close()
                #os.remove(gfile)
            FlyingBearStage.FlyingBearStageIns.setUploadFileNameSD(gjz)
        else:
            FlyingBearStage.FlyingBearStageIns.setUploadFileNameSD(gfile)            
...
class FlyingBearOutputDeviceUSB(OutputDevice): #We need an actual device to do the writing.
    def __init__(self):
        super().__init__("flyingbear_output_device_usb") #Give an ID which is used to refer to the output device.

        #Optionally set some metadata.
        self.setName("FlyingBear Output Device") #Human-readable name (you may want to internationalise this). Gets put in messages and such.
        self.setShortDescription("Send To Printer USB") #This is put on the save button.
        self.setDescription("Send gcode to Ghost USB Drive via WIFI")
        self.setIconName("save")
